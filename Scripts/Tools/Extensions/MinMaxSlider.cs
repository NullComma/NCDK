    ...
    internal sealed class MinMaxSliderDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            if (property.propertyType != SerializedPropertyType.Vector2) {
                throw new ArgumentException($"{nameof(MinMaxSlider)} underlying value type must be Vector2");
            }
            ...
            if (minLimit >= maxLimit) {
                throw new ArgumentOutOfRangeException($"{nameof(MinMaxSlider)} min limit can not greater or equal to max limit");
            }
