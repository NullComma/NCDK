...
namespace EnigmaCore {
	public class ChildrenPositionAndRotationRandomizer : MonoBehaviour {
		...
		private Vector3 GetValidPosition(Transform t, int safeOverflowCount) {
			...
			if (safeOverflowCount <= 0) {
				Debug.Log($"{nameof(ChildrenPositionAndRotationRandomizer)}: Safe overflow reached on {t.name}. Stopping loop to prevent stack overflow.", t);
				return pos;
			}
	...
	#if UNITY_EDITOR
	[CustomEditor(typeof(ChildrenPositionAndRotationRandomizer))]
	public class RandomPositionAndRotationInstantiatorEditor : Editor {
		public override void OnInspectorGUI() {
			this.DrawDefaultInspector();

			if (!(target is ChildrenPositionAndRotationRandomizer myScript)) return;
			if(GUILayout.Button(nameof(TransformExtensions.DestroyAllChildren))) {
				myScript.transform.DestroyAllChildren();
			}
            if(GUILayout.Button(nameof(ChildrenPositionAndRotationRandomizer.RandomizeScalesEditor))) {
                myScript.RandomizeScalesEditor();
            }
            if(GUILayout.Button(nameof(ChildrenPositionAndRotationRandomizer.RandomizeRotationsEditor))) {
                myScript.RandomizeRotationsEditor();
            }
			if(GUILayout.Button(nameof(ChildrenPositionAndRotationRandomizer.RandomizePositions))) {
				myScript.RandomizePositions();
			}
            if(GUILayout.Button(nameof(ChildrenPositionAndRotationRandomizer.RandomizeAll))) {
