Shader "Universal Render Pipeline/Particles/Unlit-AngleFade"
{
    Properties
    {
        ...
        _FadeEnd("Angle Fade End", Range(0.0, 1.0)) = 1.0
        ...
        [HideInInspector] _CameraFadeParams("__camerafadeparams", Vector) = (0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // -------------------------------------
            // Shader Stages
            #pragma vertex vertParticleUnlitAngleFade
            #pragma fragment fragParticleUnlitAngleFade
            ...
            // -------------------------------------
            // Includes
            #include_with_pragmas "Assets/NCDK/Graphics/Shaders/Utility/ParticlesUnlitAngleFadeInput.hlsl"
            #include_with_pragmas "Packages/com.unity.render-pipelines.universal/Shaders/Particles/ParticlesUnlitForwardPass.hlsl"

            VaryingsParticle vertParticleUnlitAngleFade(AttributesParticle input)
            {
                return vertParticleUnlit(input);
            }

            half4 fragParticleUnlitAngleFade(VaryingsParticle input) : SV_Target
            {
                half4 color = fragParticleUnlit(input);

                #if defined(_ANGLEFADE_ON)
                    #ifdef _NORMALMAP
                        half3 normalWS = half3(input.normalWS.x, input.normalWS.y, input.normalWS.z);
                        half3 viewDirWS = half3(input.normalWS.w, input.tangentWS.w, input.bitangentWS.w);
                    #else
                        half3 normalWS = input.normalWS;
                        half3 viewDirWS = input.viewDirWS;
                    #endif
                    color.a *= AngleFade(normalize(normalWS), normalize(viewDirWS));
                #endif
